...
import scala.concurrent.{Batchable, CanAwait, ExecutionContext, ExecutionException, Future, OnCompleteRunnable, TimeoutException}
...
object Promise {
  ...
  class DefaultPromise[T] private[this] (initial: AnyRef) extends AtomicReference[AnyRef](initial) with scala.concurrent.Promise[T] with scala.concurrent.Future[T] with (Try[T] => Unit) {
    ...
    override final def failed: Future[Throwable] =
      ...
      else scala.concurrent.Future.fromTry(failedFailure) // Cached instance in case of already known success
    ...
    /** Link this promise to the root of another promise.
      */
    @tailrec final def linkRootOf(target: DefaultPromise[T], link: Link[T]): Unit =
      if (this ne target) {
        val state = get()
        if (state.isInstanceOf[Try[T]]) {
    ...
    /**
      ...
      **/
    @tailrec final def unlink(resolved: Try[T]): Unit = {
      val state = get()
          ...
          (_xform: @switch) match {
            ...
            case Xform_recoverWith   =>
              if (v.isInstanceOf[Failure[F]]) {
                val f = fun.asInstanceOf[PartialFunction[Throwable, Future[T]]].applyOrElse(v.asInstanceOf[Failure[F]].exception, recoverWithFailed)
                val marker = scala.concurrent.Future.failed(new Throwable with NoStackTrace)
                if (f ne marker) {
                  if (f.isInstanceOf[DefaultPromise[T]]) f.asInstanceOf[DefaultPromise[T]].linkRootOf(this, null) else completeWith(f.asInstanceOf[Future[T]])
                  null
                } else v
            ...
            case Xform_filter        =>
              if (v.isInstanceOf[Failure[F]] || fun.asInstanceOf[F => Boolean](v.get)) v else filterFailure
            case Xform_collect       =>
              if (v.isInstanceOf[Success[F]]) Success(fun.asInstanceOf[PartialFunction[F, T]].applyOrElse(v.get, collectFailed)) else v
            case _                   =>
